"""
Thread Shed project for Learn Python 3 course at Codecademy
...
transactions_clean = []
for trans in range(len(daily_transactions_split)):
  transactions_clean.append([])
  for t in daily_transactions_split[trans]:
    transactions_clean[trans].append(t.strip())
...
# 15.Print total_sales. How much did we make today?

print("Total Sales for the day: ${:,.2f}".format(total_sales))
...
# 18. Next, iterate through thread_sold. For each item, check if it is a single color or multiple colors. If it is a single color, append that color to thread_sold_split. If it is multiple colors, first split the string around the & character and then add each color individually to thread_sold_split.
...
for color in colors:
  ...
  if count > 0:
    print("Sold {count} thread(s) of {color} thread today.".format(count=count, color=color))
  else:
    print("Sold none of {color} thread today.".format(color=color))
